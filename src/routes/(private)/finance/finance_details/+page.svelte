<script lang="ts">
	...
	const removeCategory = async (id) => {
		const res = await fetch(`/api/finance/category/` + id, {
			method: 'DELETE'
		}).then((res) => res.json());
		const { status, body } = res;
		if (status === 204) {
...
<dialog id="category_modal" class="modal">
	<!-- svelte-ignore a11y-click-events-have-key-events -->
	<!-- svelte-ignore a11y-no-static-element-interactions -->
	<div class="modal-backdrop" on:click|stopPropagation={closeCategoryDialog} />
	<div class="border rounded-lg shadow relative max-w-2xl modal-box">
		<div class="flex justify-end p-2">
			<button
				type="button"
				class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"
				on:click={closeCategoryDialog}
			>
				<svg
					class="w-5 h-5"
					fill="currentColor"
		...
		<div class="p-6 pt-0 text-center">
			...
			{#if !selectedCategory}
				...
				<div class="flex justify-center mt-5">
					...
					<form method="dialog" on:submit={closeCategoryDialog}>
						<button
							class="text-gray-900 bg-white hover:bg-gray-100 focus:ring-4 focus:ring-cyan-200 border border-gray-200 font-medium inline-flex items-center rounded-lg text-base px-3 py-2 text-center"
						>
							Nie, anuluj
			...
			{#if selectedCategory}
				<p class="text-xl mb-3 font-semibold text-gray-700 dark:text-gray-200">
					Czy na pewno chcesz usunąć kategorię <b>{selectedCategory}</b>?
				</p>
				<div class="flex justify-center mt-5">
					<button
						class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-base inline-flex items-center px-3 py-2 text-center mr-2"
					on:click={() => {removeCategory(selectedCategory)}}
						>
						Tak, usuń
					</button>
					<form method="dialog" on:submit={closeCategoryDialog}>
